...
/**
 * input_buf - Gathers connected commands from input and stores in a buffer.
	...
	if (!*len) /* If buffer is empty fill it */
	{
		...
		*buf = NULL;
		...
		if (r > 0)
		{
			...
			/* Confirm if the input has a command chaining */
			if (_strchr(*buf, ';'))
			{
				*len = r;
				info->cmd_buf = buf;
...
/**
 * get_input - Collects the input line and handles chained commands
 * @info: Structure containing relevant information.
...
ssize_t get_input(info_t *info)
{
	...
	if (len) /* If null commands are left in the chain buffer */
	{
		j = i;
		p = buf + i;

		/* Check for any occurrence of command chaining and update the position */
		...
		*buf_p = p; /* Take pointer back to current command position */
